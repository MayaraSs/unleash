...
const { FEATURE_CREATED, FEATURE_UPDATED, FEATURE_ARCHIVED, FEATURE_REVIVED, ADDON_CONFIG_CREATED, ADDON_CONFIG_UPDATED, ADDON_CONFIG_DELETED, } = require('../types/events');
const MASKED_VALUE = '*****';
const definition = {
    name: 'simple',
...
function getSetup() {
    ...
    return {
        addonService: new AddonService(stores, { getLogger, server: { unleashUrl: 'http://test' } }, tagTypeService),
        stores,
        tagTypeService,
    };
}
...
test('should reject addon config with missing required parameter when creating', async () => {
    ...
    await expect(async () => addonService.createAddon(config, 'me@mail.com')).rejects.toThrow(ValidationError);
});
test('should reject updating addon config with missing required parameter', async () => {
    ...
    await expect(async () => addonService.updateAddon(config.id, updated, 'me@mail.com')).rejects.toThrow(ValidationError);
});
