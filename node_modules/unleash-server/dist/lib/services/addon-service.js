...
const MASKED_VALUE = '*****';
class AddonService {
    constructor({ addonStore, eventStore, featureToggleStore }, config, tagTypeService) {
        ...
        this.fetchAddonConfigs = memoize(() => addonStore.getAll({ enabled: true }), { promise: true, maxAge: ADDONS_CACHE_TIME });
    }
    ...
    async validateRequiredParameters({ provider, parameters }) {
        ...
        if (requiredParamsMissing.length > 0) {
            throw new ValidationError(`Missing required parameters: ${requiredParamsMissing.join(',')} `, '');
        }
        return true;
    }
}
