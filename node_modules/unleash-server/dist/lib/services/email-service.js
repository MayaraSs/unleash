...
class EmailService {
    constructor(email, getLogger) {
        ...
        if (email && email.host) {
            ...
            if (email.host === 'test') {
                this.mailer = nodemailer_1.createTransport({ jsonTransport: true });
            }
            else {
                const connectionString = `${email.smtpuser}:${email.smtppass}@${email.host}:${email.port}`;
                this.mailer = email.secure
                    ? nodemailer_1.createTransport(`smtps://${connectionString}`)
                    : nodemailer_1.createTransport(`smtp://${connectionString}`);
            }
    ...
    async sendGettingStartedMail(name, recipient, passwordLink) {
        if (this.configured()) {
            const year = new Date().getFullYear();
    ...
    resolveTemplate(templateName, format) {
        ...
        if (fs_1.existsSync(template)) {
            return fs_1.readFileSync(template, 'utf-8');
        }
