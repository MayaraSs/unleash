...
const { FEATURE_IMPORT, DROP_FEATURES, STRATEGY_IMPORT, DROP_STRATEGIES, TAG_IMPORT, DROP_TAGS, FEATURE_TAG_IMPORT, DROP_FEATURE_TAGS, TAG_TYPE_IMPORT, DROP_TAG_TYPES, PROJECT_IMPORT, DROP_PROJECTS, } = require('../types/events');
const { readFile, parseFile, filterExisting, filterEqual, } = require('./state-util');
class StateService {
    ...
    importFile({ file, dropBeforeImport, userName, keepExisting }) {
        return readFile(file)
            ...
            .then(data => this.import({ data, userName, dropBeforeImport, keepExisting }));
    }
    ...
    async importFeatures({ features, userName, dropBeforeImport, keepExisting, }) {
        ...
        await Promise.all(features
            ...
            .map(feature => this.toggleStore.importFeature(feature).then(() => this.eventStore.store({
            type: FEATURE_IMPORT,
            createdBy: userName,
            data: feature,
        }))));
    ...
    async importProjects({ projects, userName, dropBeforeImport, keepExisting, }) {
        ...
        if (projectsToImport.length > 0) {
            const importedProjects = await this.projectStore.importProjects(projectsToImport);
    ...
    async importFeatureTags(featureTags, keepExisting, oldFeatureTags, userName) {
        ...
        if (featureTagsToInsert.length > 0) {
            const importedFeatureTags = await this.toggleStore.importFeatureTags(featureTagsToInsert);
    ...
    async export({ includeFeatureToggles = true, includeStrategies = true, includeProjects = true, includeTags = true, }) {
        return Promise.all([
            ...
            includeTags
                ...
                : Promise.resolve(),
        ]).then(([features, strategies, projects, tagTypes, tags, featureTags,]) => ({
            version: 1,
            features,
            strategies,
            projects,
