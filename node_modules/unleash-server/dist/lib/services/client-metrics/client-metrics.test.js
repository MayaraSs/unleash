...
test('should work without state', () => {
    ...
    const metrics = new UnleashClientMetrics({ clientMetricsStore }, { getLogger });
...
test('Multiple registrations of same appname and instanceid within same time period should only cause one registration', async () => {
    ...
    const clientMetrics = new UnleashClientMetrics({ clientMetricsStore, clientApplicationsStore, clientInstanceStore }, { getLogger });
    const client1 = {
        appName: 'test_app',
        instanceId: 'ava',
...
test('Same client registered outside of dedup interval will be registered twice', async () => {
    ...
    const clientMetrics = new UnleashClientMetrics({ clientMetricsStore, clientApplicationsStore, clientInstanceStore }, { getLogger, bulkInterval });
    const client1 = {
        appName: 'test_app',
        instanceId: 'client1',
    ...
    expect(firstRegistrations[0].instanceId).toBe(secondRegistrations[0].instanceId);
    jest.useRealTimers();
});
test('No registrations during a time period will not call stores', async () => {
    ...
    const metrics = new UnleashClientMetrics({ clientMetricsStore, clientApplicationsStore, clientInstanceStore }, { getLogger });
    jest.advanceTimersByTime(6 * 1000);
    expect(appStoreSpy).toHaveBeenCalledTimes(0);
    expect(bulkSpy).toHaveBeenCalledTimes(0);
    jest.useRealTimers();
});
//# sourceMappingURL=client-metrics.test.js.map