...
module.exports = class ClientMetricsService {
    constructor({ clientMetricsStore, strategyStore, featureToggleStore, clientApplicationsStore, clientInstanceStore, eventStore, }, { getLogger, bulkInterval = FIVE_SECONDS, announcementInterval: appAnnouncementInterval = FIVE_MINUTES, }) {
        this.globalCount = 0;
        this.apps = {};
        ...
        this.lastHourList.on('expire', toggles => {
            Object.keys(toggles).forEach(toggleName => {
                this.lastHourProjection.substract(toggleName, this.createCountObject(toggles[toggleName]));
            });
        });
        this.lastMinuteList.on('expire', toggles => {
            Object.keys(toggles).forEach(toggleName => {
                this.lastMinuteProjection.substract(toggleName, this.createCountObject(toggles[toggleName]));
            });
        });
        this.seenClients = {};
        this.bulkAddTimer = setInterval(() => this.bulkAdd(), bulkInterval);
        this.bulkAddTimer.unref();
        this.announceTimer = setInterval(() => this.announceUnannounced(), appAnnouncementInterval);
    ...
    async getApplication(appName) {
        ...
        const [application, instances, strategies, features,] = await Promise.all([
