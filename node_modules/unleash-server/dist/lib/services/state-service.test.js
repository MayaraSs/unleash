...
const { FEATURE_IMPORT, DROP_FEATURES, STRATEGY_IMPORT, DROP_STRATEGIES, TAG_TYPE_IMPORT, TAG_IMPORT, FEATURE_TAG_IMPORT, PROJECT_IMPORT, } = require('../types/events');
function getSetup() {
...
test('Should not import an existing tag', async () => {
    ...
    await stores.featureToggleStore.tagFeature(data.featureTags[0].featureName, {
...
test('Should drop projects before import if specified', async () => {
    ...
    await stateService.import({ data, dropBeforeImport: true });
    return expect(async () => stores.projectStore.hasProject('fancy')).rejects.toThrow(NotFoundError);
});
