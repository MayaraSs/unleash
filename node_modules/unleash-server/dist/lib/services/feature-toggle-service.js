...
const { FEATURE_ARCHIVED, FEATURE_CREATED, FEATURE_REVIVED, FEATURE_UPDATED, FEATURE_STALE_ON, FEATURE_STALE_OFF, TAG_CREATED, } = require('../types/events');
class FeatureToggleService {
    ...
    async createFeatureToggle(value, userName) {
        ...
        const feature = await this.featureToggleStore.createFeature(featureData);
    ...
    async updateToggle(updatedFeature, userName) {
        ...
        const tags = (await this.featureToggleStore.getAllTagsForFeature(updatedFeature.name)) || [];
        await this.eventStore.store({
    ...
    async archiveToggle(name, userName) {
        ...
        const tags = (await this.featureToggleStore.getAllTagsForFeature(name)) || [];
        await this.eventStore.store({
    ...
    async updateField(featureName, field, value, userName) {
        ...
        const tags = (await this.featureToggleStore.getAllTagsForFeature(featureName)) ||
            [];
        await this.eventStore.store({
            type: FEATURE_UPDATED,
            createdBy: userName,
