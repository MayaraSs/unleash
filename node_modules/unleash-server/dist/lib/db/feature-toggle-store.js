...
class FeatureToggleStore {
    ...
    async getFeatures(query = {}, fields = FEATURE_COLUMNS) {
        ...
        const queryFields = fields.map(f => mapperToColumnNames[f] ? mapperToColumnNames[f] : f);
        ...
        if (query) {
            ...
            if (query.namePrefix) {
                baseQuery = baseQuery.where('name', 'like', `${query.namePrefix}%`);
            }
        }
    ...
    async tagFeature(featureName, tag) {
        ...
        await this.db(FEATURE_TAG_TABLE)
            ...
            if (err.code === UNIQUE_CONSTRAINT_VIOLATION) {
                throw new FeatureHasTagError(`${featureName} already had the tag: [${tag.type}:${tag.value}]`);
            }
            else {
                throw err;
            }
    ...
    async getAllFeatureTags() {
        const rows = await this.db(FEATURE_TAG_TABLE).select(FEATURE_TAG_COLUMNS);
        return rows.map(row => ({
