...
class StrategyStore {
    ...
    async createStrategy(data) {
        this.db(TABLE)
            ...
            .catch(err => this.logger.error('Could not insert strategy, error: ', err));
    }
    // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
    async updateStrategy(data) {
        this.db(TABLE)
            ...
            .catch(err => this.logger.error('Could not update strategy, error: ', err));
    }
    async deprecateStrategy({ name }) {
        this.db(TABLE)
            .where({ name })
            .update({ deprecated: true })
            .catch(err => this.logger.error('Could not deprecate strategy, error: ', err));
    }
    async reactivateStrategy({ name }) {
        this.db(TABLE)
            .where({ name })
            .update({ deprecated: false })
            .catch(err => this.logger.error('Could not reactivate strategy, error: ', err));
    }
    ...
    async dropStrategies() {
        await this.db(TABLE)
            .where({ built_in: 0 }) // eslint-disable-line
            .delete()
            .catch(err => this.logger.error('Could not drop strategies, error: ', err));
    }
}
