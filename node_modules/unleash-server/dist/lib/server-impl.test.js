...
test('should call preRouterHook', async () => {
    let called = 0;
    await serverImpl.start(createTestConfig({
        server: { port: 0 },
        preRouterHook: () => {
            called++;
        },
...
test('should auto-create server on start()', async () => {
    const { server } = await serverImpl.start(createTestConfig({ server: { port: 0 } }));
...
test('should shutdown the server when calling stop()', async () => {
    const { server, stop } = await serverImpl.start(createTestConfig({ server: { port: 0 } }));
    await stop();
