...
const { FEATURE_CREATED, FEATURE_UPDATED, FEATURE_ARCHIVED, FEATURE_REVIVED, FEATURE_STALE_ON, FEATURE_STALE_OFF, } = require('../types/events');
...
class DatadogAddon extends Addon {
    ...
    async handleEvent(event, parameters) {
        const { url = 'https://api.datadoghq.com/api/v1/events', apiKey, } = parameters;
        let text;
        ...
        const tags = eventTags && eventTags.map(tag => `${tag.value}:${tag.type}`);
        const body = {
            text: `%%% \n ${text} \n %%% `,
...
This was changed by ${createdBy}.`;
    ...
    generateText(event) {
        ...
        const strategies = `**Activation strategies**: \`\`\`${YAML.safeDump(data.strategies, { skipInvalid: true })}\`\`\``;
