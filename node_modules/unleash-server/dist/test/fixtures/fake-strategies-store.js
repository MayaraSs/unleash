...
module.exports = (databaseIsUp = true) => {
    ...
    return {
        ...
        getEditableStrategies: () => Promise.resolve(_strategies.filter(s => s.editable)),
        getStrategy: name => {
        ...
        updateStrategy: strat => {
            _strategies.splice(_strategies.indexOf(({ name }) => name === strat.name), 1);
        ...
        deleteStrategy: strat => _strategies.splice(_strategies.indexOf(({ name }) => name === strat.name), 1),
        deprecateStrategy: ({ name }) => {
            const deprecatedStrat = _strategies.find(s => s.name === name);
            deprecatedStrat.deprecated = true;
            _strategies.splice(_strategies.indexOf(s => name === s.name), 1);
