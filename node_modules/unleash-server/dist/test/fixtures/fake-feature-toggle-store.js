...
module.exports = (databaseIsUp = true) => {
    ...
    return {
        ...
        updateFeature: updatedFeature => {
            _features.splice(_features.indexOf(f => f.name === updatedFeature.name), 1);
            _features.push(updatedFeature);
        },
        archiveFeature: feature => {
            _features.slice(_features.indexOf(({ name }) => name === feature.name), 1);
        ...
        reviveFeature: feature => {
            ...
            _archive.splice(_archive.indexOf(f => f.name === feature.name), 1);
        ...
        getFeatures: query => {
            ...
            if (query) {
                const activeQueryKeys = Object.keys(query).filter(t => query[t]);
                const filtered = _features.filter(feature => activeQueryKeys.every(key => {
                    if (key === 'namePrefix') {
                        return feature.name.indexOf(query[key]) > -1;
                    }
                    if (key === 'tag') {
                        return query[key].some(tagQuery => _featureTags
                            .filter(t => t.featureName === feature.name)
                            .some(tag => tag.tagType === tagQuery[0] &&
        ...
        untagFeature: event => {
            const index = _featureTags.findIndex(f => f.featureName === event.featureName &&
                f.tagType === event.type &&
                f.tagValue === event.value);
            _featureTags.splice(index, 1);
        },
        getAllTagsForFeature: featureName => Promise.resolve(_featureTags
