...
test('Should use DATABASE_URL from env', () => {
    ...
    const config = create_config_1.createConfig({});
...
test('Should use databaseURl from options', () => {
    const databaseUrl = 'postgres://u:p@localhost:5432/name';
    const config = create_config_1.createConfig({ databaseUrl });
    expect(config.db.host).toBe('localhost');
    expect(config.db.password).toBe('p');
    expect(config.db.user).toBe('u');
    expect(config.db.database).toBe('name');
...
test('should validate getLogger', () => {
    ...
    expect(() => {
        // @ts-ignore
        create_config_1.createConfig({ getLogger });
    }).toThrow();
});
test('should allow setting pool size', () => {
    ...
    const config = create_config_1.createConfig({ db });
...
test('Can set baseUriPath', () => {
    ...
    const config = create_config_1.createConfig({ server: { baseUriPath } });
    expect(config.server.baseUriPath).toBe(baseUriPath);
});
test('can convert both upper and lowercase string to enum', () => {
    expect(create_config_1.authTypeFromString('demo')).toBe(option_1.IAuthType.DEMO);
    expect(create_config_1.authTypeFromString('DEMO')).toBe(option_1.IAuthType.DEMO);
    expect(create_config_1.authTypeFromString('DeMo')).toBe(option_1.IAuthType.DEMO);
    expect(create_config_1.authTypeFromString('open_source')).toBe(option_1.IAuthType.OPEN_SOURCE);
    expect(create_config_1.authTypeFromString('OPEN_SOURCE')).toBe(option_1.IAuthType.OPEN_SOURCE);
    expect(create_config_1.authTypeFromString('ENTERPRISE')).toBe(option_1.IAuthType.ENTERPRISE);
    expect(create_config_1.authTypeFromString('enterprise')).toBe(option_1.IAuthType.ENTERPRISE);
    expect(create_config_1.authTypeFromString('custom')).toBe(option_1.IAuthType.CUSTOM);
    expect(create_config_1.authTypeFromString('CUSTOM')).toBe(option_1.IAuthType.CUSTOM);
    expect(create_config_1.authTypeFromString('none')).toBe(option_1.IAuthType.NONE);
    expect(create_config_1.authTypeFromString('NONE')).toBe(option_1.IAuthType.NONE);
    expect(create_config_1.authTypeFromString('unknown-string')).toBe(option_1.IAuthType.OPEN_SOURCE);
});
