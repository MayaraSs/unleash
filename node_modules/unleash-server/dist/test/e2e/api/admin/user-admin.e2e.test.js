...
let adminRole;
beforeAll(async () => {
    db = await database_init_1.default('user_admin_api_serial', no_logger_1.default);
    stores = db.stores;
    app = await test_helper_1.setupApp(stores);
    userStore = stores.userStore;
    accessStore = stores.accessStore;
    eventStore = stores.eventStore;
    const roles = await accessStore.getRootRoles();
    editorRole = roles.find(r => r.name === access_service_1.RoleName.EDITOR);
    adminRole = roles.find(r => r.name === access_service_1.RoleName.ADMIN);
});
afterAll(async () => {
    await app.destroy();
    await db.destroy();
...
test('creates and returns all users', async () => {
    expect.assertions(2);
    const createUserRequests = [...Array(20).keys()].map(i => app.request
...
test('creates admin-user with password', async () => {
    ...
    expect(roles[0].name).toBe(access_service_1.RoleName.ADMIN);
});
