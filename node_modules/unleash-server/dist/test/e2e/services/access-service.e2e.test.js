...
const { AccessService, RoleName, ALL_PROJECTS, } = require('../../../lib/services/access-service');
...
test('should have project admin to default project', async () => {
    const { DELETE_PROJECT, UPDATE_PROJECT, CREATE_FEATURE, UPDATE_FEATURE, DELETE_FEATURE, } = permissions;
    const user = editorUser;
...
test('should grant member CREATE_FEATURE on all projects', async () => {
    ...
    const user = editorUser;
    await accessService.addPermissionToRole(editorRole.id, permissions.CREATE_FEATURE, ALL_PROJECTS);
...
test('should remove CREATE_FEATURE on all projects', async () => {
    ...
    const user = editorUser;
    await accessService.addPermissionToRole(editorRole.id, permissions.CREATE_FEATURE, ALL_PROJECTS);
    await accessService.removePermissionFromRole(editorRole.id, permissions.CREATE_FEATURE, ALL_PROJECTS);
...
test('should grant user access to project', async () => {
    ...
    const user = editorUser;
    const sUser = await createUserEditorAccess('Some Random', 'random@getunleash.io');
    await accessService.createDefaultProjectRoles(user, project);
    const roles = await accessService.getRolesForProject(project);
    const projectRole = roles.find(r => r.name === 'Member' && r.project === project);
...
test('should not get access if not specifying project', async () => {
    ...
    const user = editorUser;
    const sUser = await createUserEditorAccess('Some Random', 'random22@getunleash.io');
    await accessService.createDefaultProjectRoles(user, project);
    const roles = await accessService.getRolesForProject(project);
    const projectRole = roles.find(r => r.name === 'Member' && r.project === project);
    await accessService.addUserToRole(sUser.id, projectRole.id);
...
test('should return role with permissions and users', async () => {
    ...
    expect(roleWithPermission.permissions.find(p => p.permission === permissions.CREATE_PROJECT)).toBeTruthy();
...
test('should return list of permissions', async () => {
    ...
    const { DELETE_FEATURE, UPDATE_FEATURE, CREATE_FEATURE, UPDATE_PROJECT, CREATE_PROJECT, } = permissions;
